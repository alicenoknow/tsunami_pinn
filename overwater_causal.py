...
class Loss:
    def __init__(
        ...
        self.epoch = 0
        self.epsilon = 0.5
        self.M = torch.triu(torch.ones((xy_points, n_points)), diagonal=1).T.flatten()


    def residuals_and_weights(self, pinn: PINN, a: float = 0.0):
        x, y, z, t = x_interior, y_interior, z_interior, t_interior
        u = f(pinn, x, y, t)
    
        u_minus_z = torch.maximum(u - z, torch.tensor(a))
        dfdx_val = dfdx(pinn, x, y, t, u)
        dfdy_val = dfdy(pinn, x, y, t, u)
        dfdx2_val = dfdx(pinn, x, y, t, u, order=2)
        dfdy2_val = dfdy(pinn, x, y, t, u, order=2)
        dzdx_val = dzdx
        dzdy_val = dzdy

        # Set dz gradients to zero where u - z is negative
        mask = (u_minus_z == a)
        dzdx_val[mask] = 0
        dzdy_val[mask] = 0

        loss = dfdt(pinn, x, y, t, u, order=2) - \
               GRAVITY * ((dfdx_val - dzdx_val) * dfdx_val + \
               u_minus_z * dfdx2_val + \
               (dfdy_val - dzdy_val) * dfdy_val + \
               u_minus_z * dfdy2_val)

        L_t = torch.mean(loss**2, dim=1)
        W = torch.exp(-self.epsilon* (self.M.to(pinn.device()) * L_t))        
        W = W.detach()
        return L_t, W

    def residual_loss(self, pinn: PINN, a: float = 0.0):
        L_t, W = self.residuals_and_weights(pinn)
        return torch.mean(W * L_t)
...
def interpolate_derivatives_to_mesh_points(x, y, grid_x, grid_y, dz_dx, dz_dy):
    ...
    dz_dx_mesh = griddata((grid_x.flatten(), grid_y.flatten()),
                          dz_dx.flatten(), points, method='linear')
    dz_dy_mesh = griddata((grid_x.flatten(), grid_y.flatten()),
                          dz_dy.flatten(), points, method='linear')
    return  torch.tensor(dz_dx_mesh, dtype=torch.float32), torch.tensor(dz_dy_mesh, dtype=torch.float32)
...
plot_running_average(loss_values, "Loss function (runnig average)", f"total_loss_ow_{NAME}")
plot_running_average(residual_loss_values, "Residual loss function (running average)", f"residual_loss_ow_{NAME}")
plot_running_average(initial_loss_values, "Initial loss function (running average)", f"initial_loss_ow_{NAME}")
plot_running_average(boundary_loss_values, "Boundary loss function (running average)", f"boundary_loss_ow_{NAME}")
