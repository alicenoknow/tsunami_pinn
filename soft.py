...
WEIGHT_INITIAL = 3000.0 # 1.0 # Weight of initial part of loss function
...
class Loss:
    def __init__(
        ...
        self.adaptive_weights = torch.tensor([self.weight_r,
                                              self.weight_b])
        self.epoch = 0
        self.call_count = 0
    ...
    def verbose(self, pinn: PINN, epoch=0):
        ...
        if self.epoch < 0:
           final_loss = 0.0001 * residual_loss + 5.0 * initial_loss + 0.0001 * boundary_loss
       	else:
            self.residual_history.append(residual_loss)
            self.boundary_history.append(boundary_loss)

            if self.call_count % self.epochs_to_make_updates == 0 and self.call_count != 0:
                self.adaptive_weights = self.softadapt_object.get_component_weights(torch.tensor(
                        self.residual_history), torch.tensor(
                        self.boundary_history), verbose=False)
                self.residual_history = []
                self.boundary_history = []

            final_loss = self.weight_i * initial_loss \
                + self.adaptive_weights[0] * residual_loss \
                + self.adaptive_weights[1] * boundary_loss
...
def train_model(
    ...
    for epoch in range(max_epochs):
        ...
        try:
            ...
            if epoch % 500 == 0:
                torch.save(nn_approximator.state_dict(), f"./results/best_{RUN_NUM}.pth")
...
torch.save(pinn_trained.state_dict(), f"results/best_soft_{NAME}.pt")

plot_running_average(loss_values, "Loss function (runnig average)", f"loss_soft_{NAME}")
plot_running_average(residual_loss_values, "Residual loss function (running average)", f"residual_loss_soft_{NAME}")
plot_running_average(initial_loss_values, "Initial loss function (running average)", f"initial_loss_soft_{NAME}")
plot_running_average(boundary_loss_values, "Boundary loss function (running average)", f"boundary_loss_soft_{NAME}")
...
def plot_frame(x_domain: List[float], 
    ...
    np.save(f"results/data_soft_{NAME}_{idx}.npy", z.detach().cpu().numpy())
