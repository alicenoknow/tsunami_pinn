...
class Loss:
    def __init__(
        ...
        self.epsilon = 0.1
        self.M = torch.triu(torch.ones((n_points*n_points, n_points)), diagonal=1).T.flatten()

    def compute_temporal_weights(self, residuals):
        with torch.no_grad():
            temporal_weights = torch.exp(-self.epsilon * torch.cumsum(residuals, dim=0))
        return temporal_weights

    def residuals_and_weights(self, pinn):
        x, y, t = get_interior_points(self.x_domain, self.y_domain, self.t_domain, self.n_points, pinn.device())
        u = f(pinn, x, y, t)
        z = self.floor(x, y)
        residuals = dfdt(pinn, x, y, t, order=2) - \
                    GRAVITY * (dfdx(pinn, x, y, t) ** 2 + \
                               (u-z) * dfdx(pinn, x, y, t, order=2) + \
                               dfdy(pinn, x, y, t) ** 2 + \
                               (u-z) * dfdy(pinn, x, y, t, order=2))

        L_t = torch.mean(residuals**2, dim=1)
    ...
    def residual_loss(self, pinn: PINN):
        ...
        #               (u-z) * dfdy(pinn, x, y, t, order=2)
        #               )
        # casual_weights = self.compute_temporal_weights(loss)
        # print(casual_weights)
        L_t, W = self.residuals_and_weights(pinn)
        return torch.mean(W * L_t)


    def initial_loss(self, pinn: PINN):
        x, y, t = get_initial_points(self.x_domain, self.y_domain, self.t_domain, self.n_points*3, pinn.device())
    ...
    for epoch in range(max_epochs):
        ...
        try:
            ...
            if loss[0].item() < best_loss:
                ...
                torch.save(nn_approximator.state_dict(), f"{DIR}/best_{NAME}.pth")
...
plot_running_average(loss_values, "Loss function (runnig average)", f"loss_{NAME}")
plot_running_average(residual_loss_values, "Residual loss function (running average)", f"residual_loss_{NAME}")
plot_running_average(initial_loss_values, "Initial loss function (running average)", f"initial_loss_{NAME}")
plot_running_average(boundary_loss_values, "Boundary loss function (running average)", f"boundary_loss_{NAME}")
