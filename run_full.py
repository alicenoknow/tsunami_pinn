...
TOTAL_TIME = 0.25                # Domain size in t axis. Always starts at 0
N_POINTS = 25                   # Number of in single asxis
...
x_initial, y_initial, t_initial = get_initial_points(x_domain, y_domain, t_domain, N_POINTS*5, pinn.device())
down, up, left, right = get_boundary_points(x_domain, y_domain, t_domain, N_POINTS*4, pinn.device())
x_interior, y_interior, t_interior = get_interior_points(x_domain, y_domain, t_domain, N_POINTS, pinn.device())
...
class Loss:
    ...
    def residual_loss(self, pinn: PINN):
        x, y, t = x_interior, y_interior, t_interior# get_interior_points(self.x_domain, self.y_domain, self.t_domain, self.n_points, pinn.device())

        u = f(pinn, x, y, t)
        z = self.floor(x, y)

        loss = dfdt(pinn, x, y, t, order=2) - GRAVITY * (
            dfdx(pinn, x, y, t) ** 2
    ...
    def initial_loss(self, pinn: PINN):
        x, y, t = x_initial, y_initial, t_initial # get_initial_points(self.x_domain, self.y_domain, self.t_domain, self.n_points*5, pinn.device())
    ...
    def verbose(self, pinn: PINN, only_initial=False):
        ...
        final_loss = \
          0.001 * self.weight_r * residual_loss + \
          10 * self.weight_i * initial_loss + \
          0.0001 * self.weight_b * boundary_loss # 5, 1000 i 1?, 0.0005
    ...
    def __call__(self, pinn: PINN, only_initial=False):
        ...
        ```
        """
        # return self.verbose(pinn, only_initial)[0]
...
def train_model(
    ...
    for epoch in range(max_epochs):
        ...
        try:
            ...
            loss: torch.Tensor = loss_fn(nn_approximator, True)
            optimizer.zero_grad()
            loss[0].backward()
            optimizer.step()
            ...
            if (epoch + 1) % 1000 == 0:
                print(f"Epoch: {epoch + 1} - Loss: {float(loss[0].item()):>7f}, Residual loss: {float(loss[1].item()):>7f}, Initial loss: {float(loss[2].item()):>7f}, Boundary loss: {float(loss[3].item()):>7f}")

        except KeyboardInterrupt:
            break

    lbfgs_optimizer = torch.optim.LBFGS(nn_approximator.parameters(), lr=1.0, history_size=50, line_search_fn="strong_wolfe", max_iter=20)
...
def plot_color(z: torch.Tensor, x: torch.Tensor, y: torch.Tensor, n_points_x, n_points_t, title, figsize=(8, 6), dpi=100, cmap="viridis"):
    ...
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    c = ax.pcolormesh(X, Y, Z, cmap=cmap, vmin=0, vmax=1.5)
...
x, y, _ = get_initial_points(x_domain, y_domain, t_domain, N_POINTS_PLOT, requires_grad=False)
z = initial_condition(x, y)
fig = plot_3D(z, x, y, N_POINTS_PLOT, N_POINTS_PLOT, "Initial condition - exact", limit=1.5)

z = pinn(x, y ,t)
fig = plot_3D(z, x, y, N_POINTS_PLOT, N_POINTS_PLOT, f"Initial condition - pinn", limit=1.5)
...
def plot(idx, t_value):
    ...
    # fig = plot_3D(z, x, y, N_POINTS_PLOT, N_POINTS_PLOT, f"PINN for t = {t_value}", limit=1.5)
    # plt.savefig('img/img_{:03d}.png'.format(idx))
    np.save(f"res/data_wide_{idx}.npy", z.detach().cpu().numpy())


time_values = np.arange(0, 0.3, 0.01)
